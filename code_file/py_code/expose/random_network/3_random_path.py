...
def _stage1_quarter(yq):
    ...
    """
    # Whether there is a sanction event in the current period
    san_list = _SAN_BY_YQ.get(yq, None)
    if not san_list:
        return [], []

    g_path = os.path.join(_PD_LOC, "network_plot", "random",f"G_{yq}.pkl")
...
def _stage2_quarter(args):
    ...
    g_path = os.path.join(_PD_LOC2, "network_plot","random", f"G_{yq}.pkl")
    with open(g_path, "rb") as f:
        G = pickle.load(f)
...
def main(n_jobs=None, max_depth=8):
    ...
    #%% -----------------------------------------------------------------------------------
    # ------------------------Stage 1: parallel computation of upstream and downstream exposure---------------------------------
    # -----------------------------------------------------------------------------------
    up_all = []
    down_all = []
    ...
    # Convert results to DataFrame (consistent with your original structure)
    ...
    # ------------------------Save data-----------------------------------------------------
    # -----------------------------------------------------------------------------------
    expose_df.to_parquet(f"{processing_data_loc}/expose_path_random.parquet", index=False)
...
#%% ---------------------------------------------------------------------------------------
# ------------------------Entry: multiprocessing on macOS must be written this way----------------------------
# ---------------------------------------------------------------------------------------
