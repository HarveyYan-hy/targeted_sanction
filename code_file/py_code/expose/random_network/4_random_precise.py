...
# ------------------------Load data----------------------------------------------
# ----------------------------------------------------------------------------------------
factset_adjust = pd.read_parquet(f"{processing_data_loc}/factset_rel_random.parquet")
path_df = pd.read_parquet(f"{processing_data_loc}/expose_path_random.parquet")
panel_df = pd.read_parquet(f"{processing_data_loc}/basic_panel.parquet",columns=['id','yq'])
full_up = pd.read_parquet(f"{processing_data_loc}/full_expose_up_random.parquet")
full_down = pd.read_parquet(f"{processing_data_loc}/full_expose_down_random.parquet")
...
#%% ---------------------------------------------------------------------------------------
# ------------------------Calculate relationship coverage period----------------------------------------------
# ----------------------------------------------------------------------------------------
...
# Calculate coverage periods for each step
...
def find_max_continuous_end(start, periods):
    if not periods or start not in periods:
        return None  # If the set is empty or start is not in the set, return None (can be adjusted to start - 1 or others if needed)
    end = start
...
#%% ---------------------------------------------------------------------------------------
# ------------------------Distinguish the two types of exposure----------------------------------------------
# ----------------------------------------------------------------------------------------
...
# Merge exposure relationships into the panel
...
# ------------------------Save data----------------------------------------------
# ----------------------------------------------------------------------------------------
panel_df.to_parquet(f"{processing_data_loc}/exp_panel_random.parquet", index=False)